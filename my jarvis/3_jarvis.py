...
import os, re, math, json, random, platform, webbrowser, pyttsx3
    ...
    try:
        ...
        for item in (data.get("RelatedTopics") or []):
            if isinstance(item, dict) and item.get("Text"):
                return item["Text"]
        return f"I couldn't find an instant answer for {query}. Try 'search {query}'."
    except Exception as e:
...
def handle(cmd_raw: str, user_name: str) -> str:
    ...
    if lcmd == "system":
        return "Here are your system details:\n" + system_info()
